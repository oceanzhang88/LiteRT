...
TextEnhancerSession* TextEnhancer_Initialize(const TextEnhancerOptions& options) {
    ...
    // --- UPDATED: Use Abseil Log (stream-style) ---
    ...
    // --- MODIFIED: Call CreateGpuOptions() ---
...
TextEnhancerStatus TextEnhancer_PreProcess(TextEnhancerSession* session, const uint8_t* rgb_data) {
    ...
    // --- ADD THIS SECTION ---
    // Create and set an event to signal that the input data is ready.
    // This is required for the RunAsync call to synchronize.
    // We use EglSyncFence as shown in the AsyncWithGlClInterop test.
    // LITERT_ASSIGN_OR_ABORT(
    //     auto input_event,
    //     litert::Event::CreateManaged(session->env->Get(), LiteRtEventTypeEglSyncFence));
    // (*session->input_buffers)[0].SetEvent(std::move(input_event));
...
TextEnhancerStatus TextEnhancer_Run(TextEnhancerSession* session, float* inference_time_ms) {
    ...
    // --- Run inference ---
    bool async = true;
    auto run_status = session->compiled_model->RunAsync(0, *session->input_buffers, *session->output_buffers, async);
