...
// --- Project Headers ---
// --- RENAMED: Point to new text_enhancer API and paths ---
...
#define LOG_TAG "TextEnhancer_CPU_Backend"
...
TextEnhancerSession* TextEnhancer_Initialize(const TextEnhancerOptions& options) {
    LOGI("TextEnhancer_Initialize (CPU Backend)...");
    ...
    if (session->original_input_width == 0 || session->original_input_height == 0) {
        LOGE("Error: input_width and input_height must be set in TextEnhancerOptions.");
        return nullptr;
    }
    ...
    LOGI("[Debug TextEnhancer_Initialize] Model Input: %dx%dx%d", session->model_input_width,
    ...
    if (session->preprocessor_type == TextEnhancerSession::PreprocessorType::kVulkan) {
        LOGI("Initializing Vulkan Pre-processor...");
        ...
        if (!session->vulkan_processor->Initialize(options.compute_shader_path,
            ...
            LOGE("Failed to initialize VulkanImageProcessor.");
            return nullptr;
        }

        if (session->model_input_channels == 3) {
            LOGI(
                "[Debug TextEnhancer_Initialize] Model needs 3ch, Vulkan outputs 4ch. Creating 4ch "
                "temp buffer.");
            session->vulkan_temp_buffer.resize(session->model_input_width *
                                               session->model_input_height * 4);
        }
    } else {
        LOGI("Using CPU Pre-processor.");
    }
...
void TextEnhancer_Shutdown(TextEnhancerSession* session) {
    if (!session) return;
    delete session;
    LOGI("TextEnhancer_Shutdown complete.");
}
...
TextEnhancerStatus TextEnhancer_PreProcess(TextEnhancerSession* session, const uint8_t* rgb_data) {
    ...
    if (session->preprocessor_type == TextEnhancerSession::PreprocessorType::kVulkan) {
        if (!session->vulkan_processor) {
            LOGE("Vulkan preprocessor not initialized.");
        ...
        if (!vk_processor->PreprocessImage(rgb_data, session->original_input_width,
                                           ...
                                           vulkan_output_ptr)) {
            LOGE("VulkanImageProcessor::PreprocessImage failed.");
        ...
        // --- CPU Pre-processing ---
        LOGI("[Debug TextEnhancer_PreProcess] Using CPU Pre-processor (ResizeImageBilinear).");
    ...
    if (!status) {
        LOGE("Failed to write to input buffer: %s", std::string(status.Error().Message()).c_str());
        return kTextEnhancerRuntimeError;
    }
...
TextEnhancerStatus TextEnhancer_PreProcess_AHB(TextEnhancerSession* session,
    ...
    if (session->preprocessor_type != TextEnhancerSession::PreprocessorType::kVulkan) {
        LOGE("AHardwareBuffer input is only supported with the Vulkan preprocessor.");
    ...
    if (!vk_processor->PreprocessImage(in_buffer, session->original_input_width,
        ...
        LOGE("VulkanImageProcessor::PreprocessImage (AHB) failed.");
        return kTextEnhancerRuntimeError;
    }
...
TextEnhancerStatus TextEnhancer_GetPreprocessedData(TextEnhancerSession* session, uint8_t** data) {
    ...
    if (session->preprocessed_data.empty()) {
        LOGE("Preprocessed data is empty. Call TextEnhancer_PreProcess first.");
        return kTextEnhancerFailed;
    }
...
TextEnhancerStatus TextEnhancer_Run(TextEnhancerSession* session, float* inference_time_ms) {
    ...
    if (!run_status) {
        LOGE("CompiledModel::Run failed: %s", std::string(run_status.Error().Message()).c_str());
        return kTextEnhancerRuntimeError;
    }
    ...
    if (profiler) {
        ...
        for (const auto& event : events) {
            ...
            if (tag == "Invoke" && event.start_timestamp_us > 0) {
                invoke_ms += event.elapsed_time_us / 1000.0;
            }
        }

        if (inference_time_ms) {
            *inference_time_ms = invoke_ms;
        }
        LOGI("Inference time (from profiler 'Invoke'): %f ms", invoke_ms);
...
TextEnhancerStatus TextEnhancer_PostProcess(TextEnhancerSession* session,
    ...
    if (!read_status) {
        LOGE("Failed to read output buffer: %s",
             std::string(read_status.Error().Message()).c_str());
        return kTextEnhancerRuntimeError;
    }

    float* data_ptr = new (std::nothrow) float[output_size];
    if (!data_ptr) {
        LOGE("Failed to allocate memory for output data.");
        return kTextEnhancerFailed;
    }
