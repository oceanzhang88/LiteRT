...
void GlCheckErrorDetail(const char* file, int line, const char* operation = "") {
  ...
  while ((error_code = glGetError()) != GL_NO_ERROR) {
    ...
    switch (error_code) {
      ...
      default: error_string = "UNKNOWN_ERROR_CODE_0x" + std::to_string(error_code); break;
    }
...
bool ImageProcessor::InitializeGL(
    ...
    const std::string& super_res_compute_shader_path) {
...
bool ImageProcessor::PreprocessInputForSuperResolution(
    GLuint input_tex_id, int output_width, int output_height,
    ...
    glUniform2i(
        glGetUniformLocation(super_res_compute_shader_program_, "output_dims"),
        output_width, output_height);

    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, input_tex_id);
    glUniform1i(
        glGetUniformLocation(super_res_compute_shader_program_, "inputTexture"), 0);
